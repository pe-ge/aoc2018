...
class Main:
    ...
    def remove_falls(self):
        def left(row_idx, col_idx):
            return '.' if col_idx-1 < 0 else self.ground[row_idx][col_idx-1]
        def right(row_idx, col_idx):
            return '.' if col_idx+1 >= self.width else self.ground[row_idx][col_idx+1]

        for row_idx in range(self.height):
            for col_idx in range(self.width):
                if self.ground[row_idx][col_idx] == '~':
                    l = left(row_idx, col_idx)

                    if l == '.':
                        self.total_water -= 1
                        self.ground[row_idx][col_idx] = '.'
                        continue

        for row_idx in range(self.height-1, -1, -1):
            for col_idx in range(self.width-1, -1, -1):
                if self.ground[row_idx][col_idx] == '~':
                    r = right(row_idx, col_idx)
